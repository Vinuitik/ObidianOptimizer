...
export default function SourceSplicePanel({ item }) {
  ...
  const kind = sourceKind(region.ref, item);
  return (
    <div className={styles.panel}>
      ...
      <div className={styles.viewer}>
        <Viewer kind={kind} region={region} item={item} />
      </div>
    </div>
  );
}

function Viewer({ kind, region, item }) {
  if (kind === 'youtube') {
  ...
  if (kind === 'video') {
    ...
    return <video key={src} className={styles.media} src={src} controls />;
  }
  if (kind === 'pdf') {
