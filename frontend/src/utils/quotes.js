// Great-men quotes for the "Are you a quitter?" guard (QuitGuard.jsx). Kept as plain
// data so it's trivial to add more — one { text, author } per line.
export const QUOTES = [
  { text: "It's not that I'm so smart, it's just that I stay with problems longer.", author: 'Albert Einstein' },
  { text: 'I have not failed. I have just found 10,000 ways that will not work.', author: 'Thomas Edison' },
  { text: 'Discipline is choosing between what you want now and what you want most.', author: 'Abraham Lincoln' },
  { text: 'It does not matter how slowly you go as long as you do not stop.', author: 'Confucius' },
  { text: 'The man who moves a mountain begins by carrying away small stones.', author: 'Confucius' },
  { text: 'Perseverance is not a long race; it is many short races one after the other.', author: 'Walter Elliot' },
  { text: 'Nothing in this world can take the place of persistence.', author: 'Calvin Coolidge' },
  { text: 'Fall seven times, stand up eight.', author: 'Japanese Proverb' },
  { text: 'A river cuts through rock not because of its power but its persistence.', author: 'James N. Watkins' },
  { text: 'The only way to do great work is to love what you do — and keep doing it.', author: 'Steve Jobs' },
  { text: 'Courage is not having the strength to go on; it is going on when you have no strength.', author: 'Theodore Roosevelt' },
  { text: 'We must all suffer one of two things: the pain of discipline or the pain of regret.', author: 'Jim Rohn' },
  { text: 'Success is the sum of small efforts repeated day in and day out.', author: 'Robert Collier' },
  { text: 'He who conquers himself is the mightiest warrior.', author: 'Confucius' },
  { text: 'The last three or four reps is what makes the muscle grow.', author: 'Arnold Schwarzenegger' },
  { text: 'Amateurs sit and wait for inspiration; the rest of us just get up and go to work.', author: 'Stephen King' },
  { text: 'What we do in life echoes in eternity.', author: 'Marcus Aurelius' },
  { text: 'You have power over your mind — not outside events. Realize this, and you will find strength.', author: 'Marcus Aurelius' },
  { text: 'The impediment to action advances action. What stands in the way becomes the way.', author: 'Marcus Aurelius' },
  { text: 'Little by little, one travels far.', author: 'J.R.R. Tolkien' },
];

export function randomQuote() {
  return QUOTES[Math.floor(Math.random() * QUOTES.length)];
}
