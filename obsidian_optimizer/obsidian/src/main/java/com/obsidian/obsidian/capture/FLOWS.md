...
| **Immediate enqueue failure (before the row ever reaches the ladder)** | `CaptureController.enqueueCapture()` catch block → `CaptureRepository.updateStatus(id, "failed")` + `common/PipelineFailureRepository.record("capture", "enqueue", …)` (added so this failure mode isn't invisible to the shared ledger, `common/FLOWS.md` "Pipeline Failures") |
