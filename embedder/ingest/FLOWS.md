...
| **Local media copy (`local:` in `## Source`)** | `jobs._ensure_local_copy()` — YouTube→`downloader.download_sync(dest=RESOURCE_DIR/media)`, local file→referenced; `synthesize._source_section()` stamps `local:`. Mount: `resources:/resources` (rw sub-bind), env `RESOURCE_DIR`. Spec: `architecture_plans/LOCAL_MEDIA_RETENTION.md` |
...
| A/V rendition (offline download) | `main.media_rendition()` `GET /media-rendition?path=&mode=` → ffmpeg transcode: video → ≤`RENDITION_HEIGHT`(480)p H.264 CRF30 + AAC64k `+faststart`; audio / `mode=audio` → mono AAC48k. Cached under `/reports/_renditions` keyed by (path,mtime,size,kind) — transcode-once. nginx `location /media-rendition` (1200s read timeout). Path-guarded. Warmed by PWA `warmMedia`, cached under the canonical player URL. Env: `RENDITION_HEIGHT`, `RENDITION_TIMEOUT_S` |
...
| **Capture-time track tagging (Phase 1b)** | `jobs.submit(track_id=)` + `jobs._link_track_item()` (per-note fan-out); `publish.create_track()` / `add_track_item()`. Full cross-cutting flow: `.../tracks/FLOWS.md#capture-time-track-tagging-phase-1b` |
